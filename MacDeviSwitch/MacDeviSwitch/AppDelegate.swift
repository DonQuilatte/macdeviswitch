...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func setupComponents() {
        ...
        let deviceSelector = DefaultDeviceSelector() // Create the new selector
        let eventHandler = SwitchControllerEventHandler() // Create the event handler

        // Initialize SwitchController with all dependencies
        switchController = SwitchController(
            ...
            eventHandler: eventHandler // Inject the event handler
        )
    ...
    private func configureApplicationMenu() {
        ...
        if mainMenu.items.count > 1 {
            ...
            if let appMenu = mainMenu.items.first?.submenu {
                ...
                // Add About item
                if let aboutItem = itemsToKeep.first(where: {
                ...
                if let preferencesItem = itemsToKeep.first(where: {
                    $0.action == #selector(AppDelegate.openPreferences(_:))
                }) {
